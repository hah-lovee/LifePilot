...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<DayScorePoint[]>(`/api/reports/day-scores?date_from=${dateFrom}&date_to=${dateTo}`)
      .then((points) => {
        setScoresByDate(Object.fromEntries(points.map((p) => [p.entry_date, p.day_score])));
      })
      .catch((err) => setError(err instanceof ApiError ? err.message : "Ошибка загрузки оценок"));
  ...
  function goToDate(isoDate: string) {
    router.push(`/diary?date=${isoDate}`);
  }
  ...
  return (
    <div className="mx-auto max-w-3xl">
          ...
          {cells.map((cell, i) => {
            ...
            return (
              <button
                ...
                className={`score-pill ${scoreClass(score)} flex aspect-[1/0.84] flex-col justify-between p-2.5 transition-shadow hover:shadow-md ${
      ...
      <div className="mt-4 flex flex-wrap items-center gap-5 text-xs text-[var(--color-muted)]">
        ...
        <span className="ml-auto text-[var(--color-faint)]">Клик по дню — открыть запись</span>
      </div>
    </div>
  );
}
