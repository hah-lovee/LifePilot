...
# The container can't discover it either: on a docker bridge network its own
# default route is the bridge (the VM), and bind-mounting the VM's
...
# env file the bot already reads. Run from cron every few minutes:
#
